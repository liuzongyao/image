...
class TestServiceSuite(object):
    def setup_class(self):
        ...
        self.k8s_namespace = self.service.global_info["$K8S_NAMESPACE"]
    ...
    def test_service(self):
        ...
        # list service
        list_result = self.service.list_service(self.k8s_namespace)
        ...
        # get service detail
        detail_result = self.service.get_service_detail(self.k8s_namespace, self.service_name)
        ...
        # update service
        update_result = self.service.update_service(self.k8s_namespace, self.service_name, service_json)
        assert update_result.status_code == 204, "更新内部路由出错:{}".format(update_result.text)

        # search service
        search_result = self.service.search_service(self.k8s_namespace, self.service_name)
