...
class TestPvcSuite(object):
    ...
    def test_pvc_use_scs(self):
        ...
        # delete pvc
        delete_result = self.pvc.delete_pvc(self.pvc.global_info["$K8S_NAMESPACE"], self.pvcusescs_name)
        assert delete_result.status_code == 204, "删除pvc失败 {}".format(delete_result.text)
        assert self.pvc.check_exists(
            self.pvc.get_common_pvc_url(self.pvc.global_info["$K8S_NAMESPACE"], self.pvcusescs_name),
            404)
        self.scs.delete_scs(self.scs_name)
